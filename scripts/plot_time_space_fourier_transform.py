...
if __name__ == '__main__':
    ...
    if True:
        ...
        idx_miss = np.where( (pos[1:] - pos[:-1]) > 1000 )[0]
    ...
    K = 2*np.pi/Lambda
    Pos = np.array(a_xk.index,dtype='float')
    ...
    ax = axes[0]
    ax.imshow(np.array(a_xk).T,aspect='auto',cmap='jet')#extent=[Pos[0], Pos[-1], K[0], K[-1]],origin='lower')

    idx = np.unique(np.linspace(0,len(Pos)-1,20).astype(int))
    ax.xaxis.set_ticks(idx)
    ax.xaxis.set_ticklabels(np.round(Pos[idx]/1e6,3))
    ax.set_xlabel(f"{args.chr} ({args.strand}) position [Mb]")

    ax.yaxis.set_ticks(np.arange(0,len(Lambda),5))
    ax.yaxis.set_ticklabels(np.round(Lambda[::5]/1e3))
